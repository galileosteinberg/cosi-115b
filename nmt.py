...
def visualize_attention(input_string, encoder, decoder, idx_dict, opts):
    ...
    for i in range(len(all_attention_weights)):
      ...
      # Set up axes
      ax.set_yticklabels([''] + list(gen_string) + ['EOS'])
      ax.set_xticklabels([''] + list(input_string) + (['EOS'] if produced_end_token else []))
...
class CausalScaledDotAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        v = self.V(values)
        unnormalized_attention = torch.bmm(q, k.transpose(1, 2)) * self.scaling_factor
        mask = torch.triu(torch.ones(unnormalized_attention.size()), diagonal=1) * self.neg_inf
        unnormalized_attention = unnormalized_attention + mask.to(unnormalized_attention.device)
        attention_weights = self.softmax(unnormalized_attention)
        context = torch.bmm(attention_weights, v)
        return context, attention_weights
